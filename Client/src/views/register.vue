            ...
            <form @submit.prevent="goToNextStep" class="step-form">
              <div class="input-group">
                ...
                <div class="glass-input">
                  <input type="text" id="username" v-model="username" placeholder="设置一个独特的昵称" required />
                </div>
              </div>
              <div class="input-group">
                <label for="email">邮箱地址</label>
                <div class="glass-input">
                  <input type="email" id="email" v-model="email" placeholder="用于接收通知和找回密码" required />
                </div>
              </div>
              <div class="input-group verification-group">
                <label for="verificationCode">邮箱验证码</label>
                <div class="glass-input with-button">
                  <input type="text" id="verificationCode" v-model="verificationCode" placeholder="6位数字" required />
          ...
          <div class="form-step step-two">
            ...
            <form @submit.prevent="handleRegister" class="step-form">
              <div class="input-group">
                ...
                <div class="glass-input">
                  <input type="password" id="password" v-model="password" @input="validatePassword" placeholder="至少8位，包含字母和数字" required />
                </div>
                <!-- 密码错误提示 -->
              ...
              <div class="input-group">
                ...
                <div class="glass-input">
                  <input type="password" id="confirmPassword" v-model="confirmPassword" @input="validatePassword" placeholder="请再次输入密码" required />
                </div>
                <!-- 确认密码错误提示 -->
                <Transition @enter="onErrorEnter" @leave="onErrorLeave">
...
defineOptions({ name: 'RegisterPage' })
  ...
  const setupIntroAnimation = () => {
    gsap.from(registerForm.value, { 
      autoAlpha: 0, y: 50, scale: 0.95, duration: 0.8, ease: 'expo.out', delay: 0.2
    });
    // 初始时，将第二步预置在舞台右侧的幕后
  ...
  const sendVerificationCode = () => {
    ...
    const interval = setInterval(() => {
      countdown.value--;
      if (countdown.value <= 0) { clearInterval(interval); isSendingCode.value = false; }
    }, 1000);
  }
  ...
  const handleRegister = () => {
    ...
    if (!validatePassword()) {
      ...
      gsap.fromTo(registerForm.value, 
        { x: 0 }, 
        { x: 10, duration: 0.6, ease: 'elastic.out(1, 0.3)' }
      );
      return; // 终止提交
    }
    ...
    gsap.to(registerForm.value, {
      ...
      onComplete: () => {
        console.log('注册成功:', { 
          email: email.value, username: username.value, password: password.value, code: verificationCode.value 
        });
      }
    });
  }
  ...
  const onErrorLeave = (el: Element, done: () => void) => {
    gsap.to(el, 
      { opacity: 0, y: -10, duration: 0.3, ease: 'power2.in', onComplete: done }
    );
  }
  ...
  .background-canvas, .trail-canvas {
      position: absolute;
      top: 0;
      left: 0;
      pointer-events: none;
  ...
  .input-group { margin-bottom: 20px; position: relative; } /* 添加 relative 以便错误提示定位 */
  .input-group label {
      display: block;
      color: var(--text-color);
      font-weight: 500;
  ...
  .glass-input.with-button input { flex-grow: 1; }
  .send-code-btn {
      flex-shrink: 0;
      padding: 8px 12px;
      margin-left: 10px;

...
const setupIntroAnimation = () => {
  const formElements = loginForm.value?.querySelectorAll(
    '.form-header, .input-group, .form-options, .login-btn, .switch-mode-link',
  )
  gsap.set(loginForm.value, { scale: 0.8, opacity: 0, y: 50 })
  gsap.set(formElements, { opacity: 0, y: 30 })
  const tl = gsap.timeline({ delay: 0.2 })
  tl.to(loginForm.value, { scale: 1, opacity: 1, y: 0, duration: 0.8, ease: 'back.out(1.7)' }).to(
    formElements,
